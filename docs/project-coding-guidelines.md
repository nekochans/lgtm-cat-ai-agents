...
- `<機能>`: `features/main`、`features/auth` などの機能ディレクトリ
- `<ソースディレクトリ>`: ソースファイルが配置されているディレクトリ名（`functions` など）
- `<ソースファイル名>`: テスト対象の関数が定義されているファイル名（拡張子なし、ケバブケース）
- `<関数名>`: テスト対象の主要関数名（ケバブケース）

**例**: `src/features/main/functions/example.ts` 内の `doExample` 関数のテスト

```text
src/features/main/functions/__tests__/example/do-example.test.ts
```
